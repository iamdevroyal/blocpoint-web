      ...
      <div class="relative w-[85%] max-w-sm h-full bg-white dark:bg-slate-900 shadow-2xl flex flex-col border-r border-slate-200 dark:border-white/5 overflow-hidden">
        ...
        <div class="p-8 space-y-8 flex-1 overflow-y-auto no-scrollbar">
          ...
          <div class="flex items-center justify-between">
            <div class="space-y-1">
              <h2 class="text-xl font-bold text-slate-800 dark:text-white tracking-tight">Trade Details</h2>
              <p class="text-[9px] font-bold text-primary uppercase tracking-[0.2em]">Blockchain Settlement</p>
            </div>
          ...
          <div class="p-6 bg-emerald-500/5 rounded-[2rem] border border-emerald-500/10 flex items-center gap-4">
            ...
            <div>
              <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Execution Status</p>
              <p class="text-sm font-bold text-emerald-500 uppercase tracking-widest">Settled On-Chain</p>
            </div>
          </div>
          ...
          <div class="space-y-6">
            <div class="flex items-center gap-5">
              <div class="w-20 h-20 rounded-[2rem] bg-slate-50 dark:bg-white/5 p-4 border border-black/5 flex items-center justify-center shadow-xl">
                 <span class="text-4xl">{{ order?.coin === 'USDT' ? '🟢' : '🔵' }}</span>
              </div>
              <div class="space-y-1">
                <h3 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">{{ order?.type }} {{ order?.coin }}</h3>
            ...
            <div class="space-y-3 pt-4">
              <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Blockchain Hash (TXID)</p>
              <div class="p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border-2 border-slate-100 dark:border-white/10 break-all">
                <p class="text-[10px] font-mono font-bold text-slate-500 leading-relaxed">
                  0x{{ Math.random().toString(16).substr(2, 40).toUpperCase() }}
                </p>
              </div>
              <button class="flex items-center gap-2 text-[9px] font-bold text-primary uppercase tracking-[0.2em] px-1 group">
                <span>View On Explorer</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" class="group-hover:translate-x-1 transition-transform"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
              </button>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 pt-6 border-t border-slate-100 dark:border-white/5">
              <div v-for="(item, key) in {
                'Trade ID': 'BP-' + Math.random().toString(36).substr(2, 8).toUpperCase(),
                'Settled Value': '$' + (order?.amount / 1450).toFixed(2),
                'Exchange Rate': '₦1,450 / $1',
                'Network Fee': '$0.85',
                'Method': 'Settlement Wallet',
                'Timestamp': 'Feb 26, 2026 • 08:35 AM'
        ...
        <div class="p-8 pt-0 mt-auto">
          <button class="w-full h-14 bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[10px] font-bold rounded-2xl shadow-xl active:scale-95 transition-all uppercase tracking-widest">
            Send Reciept
          </button>
        </div>
      </div>
