        ...
        <div class="bg-white/70 dark:bg-slate-900/50 backdrop-blur-xl border border-white dark:border-white/5 rounded-3xl p-7 shadow-2xl shadow-slate-200/50 dark:shadow-none transition-colors">
          <div class="mb-8">
            <h1 class="text-2xl font-extrabold tracking-tight">
              {{ step === 1 ? 'Recover Access' : step === 2 ? 'Security Code' : 'New Security PIN' }}
            </h1>
            <p class="text-slate-500 dark:text-slate-400 font-medium text-sm mt-1">
              {{ step === 1 ? 'Enter your registered phone number' : step === 2 ? `Verification code sent to ${form.phone}` : 'Set a new 4-digit PIN for your account' }}
            </p>
          </div>
          ...
          <form @submit.prevent="handleForgotPin" class="space-y-6">
              ...
              <div :key="step" class="space-y-6">
                ...
                <div v-if="step === 3" class="space-y-4">
                  <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 ml-1">New Secure PIN</label>
                    <input 
                      v-model="form.newPin"
                      type="password" 
                      inputmode="numeric"
                  ...
                  <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 ml-1">Confirm New PIN</label>
                    <input 
            ...
            <div class="flex items-center gap-3 pt-4">
              <button 
                ...
                type="button"
                @click="prevStep"
                class="w-16 py-4 rounded-2xl border-2 border-slate-200 dark:border-white/10 bg-white/50 dark:bg-slate-800/50 flex items-center justify-center text-slate-500 dark:text-slate-400 transition-all active:scale-95"
              >
              ...
              <button 
                type="submit" 
                :disabled="isLoading"
                class="flex-1 py-4 rounded-2xl bg-primary hover:bg-indigo-500 text-white font-bold text-lg shadow-lg shadow-primary/25 transition-all active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed group relative overflow-hidden"
              >
          ...
          <div class="mt-8 text-center pt-6 border-t border-slate-100 dark:border-white/5">
            <button 
              @click="router.push('/auth/login')"
              class="text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-primary transition-colors flex items-center justify-center gap-2 mx-auto"
            >
